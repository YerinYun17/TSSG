function [im,jiDim,jiUL,jiLR,resolu,ZC]=enviread(filename)
...
% jiLR 임의추가 LowRight corner of the pixel (x,y)
...
indx_xLR = strmatch('map',geo_str)+8;
indx_yLR = strmatch('map',geo_str)+9;
indx_xreso = strmatch('map',geo_str)+10;
indx_yreso = strmatch('map',geo_str)+11;
indx_zc = strmatch('map',geo_str)+12;
...
jiLR(1)=str2double(geo_str(indx_xLR));
jiLR(2)=str2double(geo_str(indx_yLR));
